...
test("issue #90: FAILURE_KINDS is exactly the union of the intrinsic and transient sets -- every kind is classified, none twice", () => {
  assert.deepEqual(
    FAILURE_KINDS,
    [...INTRINSIC_FAILURE_KINDS, ...TRANSIENT_FAILURE_KINDS],
  ...
  assert.equal(new Set(FAILURE_KINDS).size, FAILURE_KINDS.length, "no kind appears in both sets");
  for (const kind of FAILURE_KINDS) {
    assert.equal(
      INTRINSIC_FAILURE_KINDS.includes(kind) !== TRANSIENT_FAILURE_KINDS.includes(kind),
      true,
      `'${kind}' must be in exactly one of the two sets -- an unclassified kind is an unanswered retryability question`,
    );
  }
});
