  ...
  for (const c of cells) {
    ...
    bodies.set(key, {
      costRows: [
        ...
        // voyage tokens are not what a generation slot's rate prices, and a
        // per-slot token mean that included them would misattribute embedder
        // volume to the generation model. Written in BOTH shapes a store can
        // hold it -- the flat `model`/`input_tokens` row lib/accounting.mjs
        // writes today, and a tokens_by_model entry, which is the shape the
        // exclusion in measureArmTokens actually guards against.
        { cellKey: key, model: "voyage-4-lite", input_tokens: 99999 },
        { cellKey: key, tokens_by_model: { "voyage-4-lite": { input_tokens: 99999, output_tokens: 88888 } } },
      ],
    });
  }
...
test("measureArmTokens: means per completed cell, embedder rows excluded, non-completed rows excluded", () => {
  ...
  assert.equal(m.get("X").outputTokens, 2000, "the embedder rows -- in EITHER shape -- never entered the mean");
});
