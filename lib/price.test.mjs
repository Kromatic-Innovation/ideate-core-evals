...
import {
  ...
  priceRowByProvider,
  priceRowsByProvider,
  LEGACY_PRICING_REGIME_FALLBACK,
  EFFORT_OUTPUT_MULTIPLIER,
  UNCALIBRATED_HIGH_FACTOR,
  priceJudgeLeg,
} from "./price.mjs";
...
// ── Sonnet 5 has ONE rate, and the dated-regime machinery still works ────────
//
// This block used to assert a $3/$15 standard rate with a $2/$10 intro window
// closing 2026-08-31. #143 established first-party that the announced
// increase was cancelled and $2/$10 is the standard price, so the window
// never existed as a rate CHANGE and the row now carries no regime at all.
// Two things therefore need covering, and they are different things: that
// Sonnet 5 prices at one rate on both sides of that former boundary, and that
// resolveBaseRate's dated-regime path still works for the next real one.
describe("claude-sonnet-5 rate (#143)", () => {
  const MTOK_ROW = (timestamp) =>
    costRow({
      cellKey: "k", timestamp, billing_mode: "api",
      model: "claude-sonnet-5", input_tokens: 1_000_000, output_tokens: 1_000_000,
    });

  test("prices $2/$10 per MTok on BOTH sides of the former 2026-08-31 boundary", () => {
    // The load-bearing assertion of the whole fix. A row dated after the old
    // cutoff is the one that was priced 50% high; a row dated before it is
    // the one a naive "just delete the intro rate" fix would have broken.
    for (const ts of ["2026-07-01T00:00:00Z", "2026-08-30T00:00:00Z", "2026-09-01T00:00:00Z", "2027-01-01T00:00:00Z"]) {
      const priced = priceRow(MTOK_ROW(ts));
      assert.ok(
        Math.abs(priced.total - (2.0 + 10.0)) < 1e-9,
        `claude-sonnet-5 must price $2 in / $10 out per MTok at ${ts}, got $${priced.total}`,
      );
    }
  });

  test("the pinned row carries no dated rate regime", () => {
    const entry = RATE_TABLE["claude-sonnet-5"];
    assert.equal(entry.introUntil, undefined, "a regime whose two sides are the same rate is a fiction, not history");
    assert.equal(entry.introRate, undefined);
    assert.equal(entry.in, 2.0);
    assert.equal(entry.out, 10.0);
  });

  test("resolveBaseRate's dated-regime path still applies, against an injected table", () => {
    // No LIVE row carries a regime any more, so this exercises the machinery
    // the way the straddle guards in runner.mjs do: with a fixture table.
    // Without this test, dropping Sonnet 5's regime would have quietly left
    // the whole date-window path uncovered.
    const fixture = {
      "model-with-a-real-rate-change": {
        in: 9.0, out: 90.0,
        introUntil: "2026-08-31",
        introRate: { in: 1.0, out: 10.0 },
        source: "fixture", date: "2026-09-08",
      },
    };
    const at = (timestamp) =>
      priceRow(
        costRow({
          cellKey: "k", timestamp, billing_mode: "api",
          model: "model-with-a-real-rate-change", input_tokens: 1_000_000, output_tokens: 1_000_000,
        }),
        fixture,
      ).total;
    assert.ok(Math.abs(at("2026-08-01T00:00:00Z") - 11.0) < 1e-9, "before the cutoff, the intro rate applies");
    assert.ok(Math.abs(at("2026-09-15T00:00:00Z") - 99.0) < 1e-9, "after it, the standard rate does");
  });

  test("pricing resolves the regime from the ROW's timestamp, never from wall-clock now", () => {
    const row = costRow({
...
// ── The Anthropic rows agree with the check that produced them (#143) ────────
describe("ANTHROPIC_PRICE_VERIFICATION", () => {
  test("every Anthropic RATE_TABLE row matches the recorded first-party check", () => {
    // The failure #143 describes is a pinned rate drifting from its source
    // with nothing in the repo able to notice. This does not detect a rate
    // change at the vendor -- nothing offline can -- but it does stop the
    // table and its own verification record from disagreeing, which is how a
    // half-applied correction would look.
    const found = ANTHROPIC_PRICE_VERIFICATION.modelsFoundUsdPerMTok;
    for (const [model, entry] of Object.entries(RATE_TABLE)) {
      if (!model.startsWith("claude-")) continue;
      assert.ok(found[model], `RATE_TABLE has an Anthropic row '${model}' that the verification record does not cover`);
      assert.equal(entry.in, found[model].in, `${model} input rate disagrees with the verification record`);
      assert.equal(entry.out, found[model].out, `${model} output rate disagrees with the verification record`);
      assert.equal(entry.verified, true, `${model} was verified first-party and must say so`);
      assert.equal(entry.date, ANTHROPIC_PRICE_VERIFICATION.checkedAt, `${model}'s date must be the check date`);
    }
  });

  test("the record names the correction it was made for, and what it left alone", () => {
    const c = ANTHROPIC_PRICE_VERIFICATION.corrections.find((x) => x.model === "claude-sonnet-5");
    assert.ok(c, "the Sonnet 5 correction is the reason this record exists");
    assert.deepEqual({ in: c.now.in, out: c.now.out }, { in: 2.0, out: 10.0 });
    assert.equal(c.was.in, 3.0);
    assert.ok(ANTHROPIC_PRICE_VERIFICATION.notCovered.includes("voyage-4-lite"), "the one row this check did NOT cover stays named");
  });
});
...
// ── issue #119: pricing_regime resolves the rate PER ROW ──────────────────
describe("pricing_regime -- per-row regime resolution (issue #119)", () => {
  const batchRow = costRow({
    cellKey: "k1", timestamp: "2026-07-15T00:00:00Z", billing_mode: "api",
    pricing_regime: "batch", model: "claude-opus-5", input_tokens: 1_000_000, output_tokens: 0,
  });
  const singleRow = costRow({
    cellKey: "k2", timestamp: "2026-07-15T00:00:00Z", billing_mode: "api",
    pricing_regime: "single", model: "claude-opus-5", input_tokens: 1_000_000, output_tokens: 0,
  });
  const legacyRow = costRow({
    cellKey: "k3", timestamp: "2026-07-15T00:00:00Z", billing_mode: "api",
    model: "claude-opus-5", input_tokens: 1_000_000, output_tokens: 0,
  });

  test("a row's own pricing_regime resolves the rate, regardless of the opts.batch fallback", () => {
    // batchRow says "batch" -- priced at half rate even though opts asks for
    // the single (full) rate; singleRow says "single" -- priced at full rate
    // even though opts asks for the batch (half) rate. The row's own fact
    // wins outright in both directions.
    const batchPriced = priceRow(batchRow, RATE_TABLE, { batch: false });
    assert.ok(Math.abs(batchPriced.total - 2.5) < 1e-9, "row says batch -> half rate, opts.batch is ignored");
    assert.equal(batchPriced.pricing_regime, "batch");
    assert.equal(batchPriced.pricedUnderFallback, false);

    const singlePriced = priceRow(singleRow, RATE_TABLE, { batch: true });
    assert.ok(Math.abs(singlePriced.total - 5.0) < 1e-9, "row says single -> full rate, opts.batch is ignored");
    assert.equal(singlePriced.pricing_regime, "single");
    assert.equal(singlePriced.pricedUnderFallback, false);
  });

  test("a legacy row (no pricing_regime) falls back to opts.batch, and is flagged pricedUnderFallback", () => {
    const priced = priceRow(legacyRow, RATE_TABLE, { batch: true });
    assert.ok(Math.abs(priced.total - 2.5) < 1e-9, "legacy row uses the FALLBACK, not its own (absent) fact");
    assert.equal(priced.pricedUnderFallback, true);
    assert.equal(priced.pricing_regime, "batch", "reports which regime the fallback actually resolved to");
  });

  test("priceRow's own default IS LEGACY_PRICING_REGIME_FALLBACK", () => {
    const priced = priceRow(legacyRow, RATE_TABLE);
    assert.equal(priced.pricing_regime, LEGACY_PRICING_REGIME_FALLBACK);
    assert.equal(LEGACY_PRICING_REGIME_FALLBACK, "single", "the reconciled default: never under-prices a legacy row");
  });

  test("priceRows counts legacyPricingRowCount and names the fallback used", () => {
    const { legacyPricingRowCount, legacyPricingFallbackRegime } = priceRows([batchRow, singleRow, legacyRow], RATE_TABLE, { batch: false });
    assert.equal(legacyPricingRowCount, 1, "only the legacy row is unaccounted-for by a recorded fact");
    assert.equal(legacyPricingFallbackRegime, "single");
  });

  test("priceRows reports zero legacy rows when every row carries its own regime", () => {
    const { legacyPricingRowCount } = priceRows([batchRow, singleRow], RATE_TABLE);
    assert.equal(legacyPricingRowCount, 0);
  });

  // ── The AC's own required proof: a mixed-regime store prices differently
  // from pricing it wholly as either single regime -- a single-regime store
  // would pass ANY implementation trivially, batch-blind or not. ──────────
  test("AC: a store holding BOTH regimes prices a total that differs from pricing it wholly as either regime", () => {
    const mixedStore = [batchRow, singleRow]; // one of each, same model/tokens/timestamp
    const mixedTotal = priceRows(mixedStore, RATE_TABLE).totalUsd; // per-row: $2.5 + $5.0 = $7.5

    // Pricing the SAME two rows wholly as batch (stripping their own facts,
    // simulating the pre-#119 store-wide-flag behaviour) or wholly as single:
    const strippedRows = mixedStore.map(({ pricing_regime, ...rest }) => rest);
    const whollyBatch = priceRows(strippedRows, RATE_TABLE, { batch: true }).totalUsd; // $2.5 + $2.5 = $5.0
    const whollySingle = priceRows(strippedRows, RATE_TABLE, { batch: false }).totalUsd; // $5.0 + $5.0 = $10.0

    assert.ok(Math.abs(mixedTotal - 7.5) < 1e-9);
    assert.notEqual(mixedTotal, whollyBatch);
    assert.notEqual(mixedTotal, whollySingle);
    // And the mixed-regime total sits strictly BETWEEN the two uniform
    // totals -- the direct, measurable signature of "priced at the wrong
    // rate for exactly one of the two regimes" this issue exists to fix.
    assert.ok(mixedTotal > whollyBatch && mixedTotal < whollySingle);
  });
});
...
// ── isNonProviderModel / embedder exclusion (issue #64 follow-up, cwc PR #72
// review): Phase 0 (#69) writes real `voyage-4-lite` cost rows to the SAME
// store a provider ceiling reads from. A KNOWN non-provider model must be
// cleanly excluded from `byProvider` -- never thrown on (that would hard-fail
// a resumed study on a correctly-written row, on the exact path whose job is
// to stop an unbounded bill), and never silently dropped (it's real money).
// An UNKNOWN model must still throw -- that guard catches a real typo'd or
// unregistered model id and must not be weakened.
describe("isNonProviderModel — distinguishing a KNOWN non-provider model from an unrecognized one", () => {
  test("a voyage-* id is a known non-provider model", () => {
    assert.equal(isNonProviderModel("voyage-4-lite"), true);
  });

  test("a claude-*/openai-*/gpt-* id is NOT a non-provider model", () => {
    assert.equal(isNonProviderModel("claude-sonnet-5"), false);
    assert.equal(isNonProviderModel("gpt-5.6-terra"), false);
  });

  test("an unrecognized id (no known provider prefix, not a known non-provider prefix either) is NOT a non-provider model -- it must still reach providerOf's throw", () => {
    // This is the exact case the fix must NOT quietly swallow: a typo'd or
    // unregistered generator/judge model id (e.g. a config typo of
    // 'cloud-sonnet-5' or a brand-new model nobody added a prefix rule for)
    // must still be classified as "needs providerOf to throw", never routed
    // around it as if it were a deliberately-excluded model.
    assert.equal(isNonProviderModel("totally-unknown-model-id"), false);
  });
});

describe("priceRowByProvider / priceRowsByProvider — a Phase 0-shaped embedder row is excluded, never thrown on, never silently dropped", () => {
  // Exactly the shape evals/metrics/phase0.mjs writes (git show origin/develop:evals/metrics/phase0.mjs):
  // costRow({ cellKey, timestamp, billing_mode: "api", model: embedder.modelId, input_tokens }) -- no output_tokens (embeddings have none).
  const phase0Row = costRow({
    cellKey: "phase0/dat-replication",
  ...
  test("a real Phase 0-shaped voyage-4-lite row does NOT throw through priceRowByProvider (the pre-fix behavior)", () => {
    // Before the fix, this would throw inside providerOf('voyage-4-lite') --
    // exactly the failure mode reported: a correctly-written cost row hard-
    // failing the very code path (--max-spend) whose job is to stop an
    // unbounded bill.
    assert.doesNotThrow(() => priceRowByProvider(phase0Row));
  });

  test("the embedder row's byProvider is empty -- it is not gated by EITHER provider ceiling", () => {
    const { byProvider } = priceRowByProvider(phase0Row);
    assert.deepEqual(byProvider, {}, "voyage-4-lite must not land in anthropic or openai's bucket");
  });

  test("the embedder row's spend is NOT reported as a missing rate -- it has a real RATE_TABLE entry and is priced, just excluded from byProvider", () => {
    const { hasMissingRate, missingRateModels } = priceRowByProvider(phase0Row);
    assert.equal(hasMissingRate, false, "voyage-4-lite has a RATE_TABLE entry -- excluding it from byProvider must not masquerade as an unpriced model");
    assert.deepEqual(missingRateModels, []);
  });

  test("the embedder row's spend is surfaced, not silently dropped: excludedNonProviderUsd is positive and excludedNonProviderModels names it", () => {
    const { excludedNonProviderUsd, excludedNonProviderModels } = priceRowByProvider(phase0Row);
    assert.ok(excludedNonProviderUsd > 0, "the embedder row is real, priced money -- it must show up SOMEWHERE, just not in byProvider");
    assert.deepEqual(excludedNonProviderModels, ["voyage-4-lite"]);
  });

  test("an UNKNOWN model (no known provider prefix, not voyage-*) still throws through priceRowByProvider -- the exclusion must not quietly swallow a real config error", () => {
    const row = costRow({
      cellKey: "cell-unknown",
      timestamp: "2026-08-01T00:00:00Z",
      billing_mode: "api",
      model: "totally-unknown-provider-model",
      input_tokens: 100,
      output_tokens: 50,
    });
    // priceRow() itself would mark this missingRate (no RATE_TABLE entry) --
    // that path already doesn't throw (documented, pre-existing behavior).
    // To specifically exercise "an unrecognized model with a RATE, routed
    // past isNonProviderModel, must still hit providerOf's throw", inject a
    // rate table that DOES carry a rate for it -- otherwise this test would
    // pass for the wrong reason (missingRate short-circuiting before
    // isNonProviderModel/providerOf are ever consulted).
    const rateTableWithUnknownModel = { ...RATE_TABLE, "totally-unknown-provider-model": { in: 1, out: 1, source: "test", date: "2026-08-01" } };
    assert.throws(() => priceRowByProvider(row, rateTableWithUnknownModel), /cannot infer a provider/);
  });

  test("priceRowsByProvider aggregates excludedNonProviderUsd/Models across MULTIPLE rows -- a mutation testing library, not just a single row's pass-through", () => {
    const genRow = costRow({
      cellKey: "cell-gen",
      timestamp: "2026-08-01T00:00:00Z",
      billing_mode: "api",
      model: "claude-sonnet-5",
    ...
    const secondPhase0Row = costRow({
      cellKey: "phase0/negative-controls",
      timestamp: "2026-09-01T12:05:00.000Z",
      billing_mode: "api",
      model: "voyage-4-lite",
      input_tokens: 30_000,
    });
    const { byProvider, excludedNonProviderUsd, excludedNonProviderModels } = priceRowsByProvider([genRow, phase0Row, secondPhase0Row]);

    assert.deepEqual(Object.keys(byProvider), ["anthropic"], "only the generation row contributes to byProvider");
    assert.ok(byProvider.anthropic > 0);

    // Independently derive the expected embedder total via priceRowByProvider
    // on each embedder row alone, so this assertion would still fail if the
    // aggregator summed only the LAST embedder row (an accumulator bug: `=`
    // instead of `+=`) or dropped one entirely.
    const expectedEmbedderUsd = priceRowByProvider(phase0Row).excludedNonProviderUsd + priceRowByProvider(secondPhase0Row).excludedNonProviderUsd;
    assert.equal(excludedNonProviderUsd, expectedEmbedderUsd, "both embedder rows' spend must be summed, not just the last one");
    assert.deepEqual(excludedNonProviderModels, ["voyage-4-lite"], "distinct model list, not one entry per row");
  });
});

// ── issue #169: runnerPriceGrid gets a model of `effort` ─────────────────────

test("issue #169: an `effort: max` arm projects ABOVE an otherwise identical `effort: high` arm", () => {
  // The defect verbatim: `S1C-RICH`'s two `effort: max` slots projected the
  // same as `high`, so the 144-cell re-collect's $41.59 estimate was 2.5x low
  // in exactly the arm the ceiling most needed to price.
  const arms = {
    HI: { mode: "solo", slots: [{ persona: "solo", model: "claude-sonnet-5", effort: "high" }] },
    MAX: { mode: "solo", slots: [{ persona: "solo", model: "claude-sonnet-5", effort: "max" }] },
    LOW: { mode: "solo", slots: [{ persona: "solo", model: "claude-sonnet-5", effort: "low" }] },
    NONE: { mode: "solo", slots: [{ persona: "solo", model: "claude-sonnet-5" }] },
  };
  const grid = runnerPriceGrid();
  const price = (armId) => grid([{ key: `c-${armId}`, armId }], arms).usd;

  assert.ok(price("MAX") > price("HI"), "a max-effort arm costs more than a high-effort one");
  assert.ok(price("LOW") < price("HI"), "and a low-effort arm costs less");
  assert.equal(price("NONE"), price("HI"), "an effort-free slot prices as high -- Appendix F item 5's registered equivalence");

  // Only the OUTPUT half scales: effort buys reasoning tokens, not a longer
  // prompt. The measured input was flat at ~362 tokens across all three levels.
  const inOnly = ((2500 / 1_000_000) * RATE_TABLE["claude-sonnet-5"].in) * 0.5;
  const outHi = price("HI") - inOnly;
  const outMax = price("MAX") - inOnly;
  assert.ok(Math.abs(outMax / outHi - EFFORT_OUTPUT_MULTIPLIER.max) < 1e-9, "the output half scales by exactly the multiplier");
});

test("issue #169: an all-`high` arm's projection is byte-identical to the pre-effort-term number", () => {
  // The effort term must be a pure addition for every arm that was already
  // priced correctly, or it silently re-baselines every existing ceiling.
  const arms = { P: { mode: "panel", slots: [{ model: "claude-sonnet-5", effort: "high" }, { model: "claude-sonnet-5", effort: "high" }] } };
  const armsNoEffort = { P: { mode: "panel", slots: [{ model: "claude-sonnet-5" }, { model: "claude-sonnet-5" }] } };
  const grid = runnerPriceGrid();
  assert.equal(grid([{ key: "c1", armId: "P" }], arms).usd, grid([{ key: "c1", armId: "P" }], armsNoEffort).usd);
});

test("issue #169: an UNRECOGNISED effort level throws rather than pricing at the base rate", () => {
  const arms = { W: { mode: "solo", slots: [{ model: "claude-sonnet-5", effort: "ludicrous" }] } };
  assert.throws(() => runnerPriceGrid()([{ key: "c1", armId: "W" }], arms), /no output-token multiplier for effort level 'ludicrous'/);
});

test("issue #169: an injected calibration OVERRIDES the static effort table", () => {
  const arms = { MAX: { mode: "solo", slots: [{ model: "claude-sonnet-5", effort: "max" }] } };
  const stock = runnerPriceGrid()([{ key: "c1", armId: "MAX" }], arms).usd;
  const fitted = runnerPriceGrid(undefined, { calibration: { effortMultipliers: { high: 1, max: 20 } } })([{ key: "c1", armId: "MAX" }], arms).usd;
  assert.ok(fitted > stock, "a store-fitted 20x multiplier prices above the static 6.16x");
});

test("issue #169: a tier-1 per-arm fit replaces the structural estimate AND suppresses the effort multiplier", () => {
  // Double-counting check: the measurement already contains whatever effort
  // did to this arm, so re-applying the multiplier on top would inflate it.
  const arms = { MAX: { mode: "solo", slots: [{ model: "claude-sonnet-5", effort: "max" }] } };
  const cells = [{ key: "c1", armId: "MAX" }];
  const calibration = { perArm: { MAX: { inputTokens: 2500, outputTokens: 1500, n: 10 } } };
  const fitted = runnerPriceGrid(undefined, { calibration })(cells, arms);
  const plainHigh = runnerPriceGrid()(cells, { MAX: { mode: "solo", slots: [{ model: "claude-sonnet-5", effort: "high" }] } });
  assert.equal(fitted.usd, plainHigh.usd, "fitted at exactly the solo estimate's own tokens -> exactly the high-effort price");
});

test("issue #169: a fitted cell's range is degenerate; an unfitted cell's usdHigh is the stated floor factor above it", () => {
  const arms = { A: { mode: "solo", slots: [{ model: "claude-sonnet-5" }] } };
  const cells = [{ key: "c1", armId: "A" }];

  const unfitted = runnerPriceGrid()(cells, arms);
  assert.equal(unfitted.calibrated, false);
  assert.deepEqual(unfitted.uncalibratedArmIds, ["A"]);
  assert.ok(Math.abs(unfitted.usdHigh / unfitted.usd - UNCALIBRATED_HIGH_FACTOR) < 1e-9);
  assert.equal(unfitted.breakdown[0].calibrated, false);

  const fitted = runnerPriceGrid(undefined, { calibration: { perArm: { A: { inputTokens: 2500, outputTokens: 1500, n: 9 } } } })(cells, arms);
  assert.equal(fitted.calibrated, true);
  assert.equal(fitted.usdHigh, fitted.usd, "a measurement is not padded");
  assert.deepEqual(fitted.uncalibratedArmIds, []);
});

test("issue #169: `calibrated` is false when ANY planned arm is unfitted, not only when all are", () => {
  const arms = {
    A: { mode: "solo", slots: [{ model: "claude-sonnet-5" }] },
    B: { mode: "solo", slots: [{ model: "claude-sonnet-5" }] },
  };
  const grid = runnerPriceGrid(undefined, { calibration: { perArm: { A: { inputTokens: 100, outputTokens: 100, n: 9 } } } });
  const out = grid([{ key: "c1", armId: "A" }, { key: "c2", armId: "B" }], arms);
  assert.equal(out.calibrated, false, "one unfitted arm makes the whole projection a floor");
  assert.deepEqual(out.uncalibratedArmIds, ["B"]);
  assert.deepEqual(out.calibratedArmIds, ["A"]);
});

test("issue #169: priceJudgeLeg prices a leg exactly as runnerPriceGrid's judge term does", () => {
  // One rule, two call sites (the pre-flight projection and the runner's new
  // mid-flight gate). If these ever diverge a leg is admitted at one price and
  // counted at another, which is how a ceiling silently stops binding.
  const arms = { A: { mode: "solo", slots: [{ model: "claude-sonnet-5" }] } };
  const leg = { model: "claude-opus-5", candidateCount: 30 };
  const withJudge = runnerPriceGrid(undefined, { judgeLegsFor: () => [leg] })([{ key: "c1", armId: "A" }], arms).usd;
  const without = runnerPriceGrid()([{ key: "c1", armId: "A" }], arms).usd;
  assert.ok(Math.abs(withJudge - without - priceJudgeLeg(leg).usd) < 1e-12);
});

test("issue #169: priceJudgeLeg scales with candidate count, applies the batch discount, and fails loud on a missing rate", () => {
  const one = priceJudgeLeg({ model: "claude-opus-5", candidateCount: 10 }).usd;
  const six = priceJudgeLeg({ model: "claude-opus-5", candidateCount: 60 }).usd;
  assert.ok(Math.abs(six / one - 6) < 1e-9);
  assert.ok(priceJudgeLeg({ model: "claude-opus-5", candidateCount: 10 }, undefined, { batch: false }).usd > one);
  assert.deepEqual(Object.keys(priceJudgeLeg({ model: "gpt-5.6-terra", candidateCount: 5 }).byProvider), ["openai"]);
  assert.throws(() => priceJudgeLeg({ model: "no-such-model-169", candidateCount: 5 }), /no RATE_TABLE entry/);
  assert.throws(() => priceJudgeLeg({}), /leg.model/);
});
