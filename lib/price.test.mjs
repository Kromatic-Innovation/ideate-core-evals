...
describe("runnerPriceGrid — optional DI adapter for evals/harness/runner.mjs", () => {
  ...
  // ── issue #63: the judge term is OPT-IN and adds to every planned cell ────
  test("with no judgeModels supplied, the projection is byte-identical to before the judge term existed", () => {
  ...
  test("judgeModels adds one judge leg's estimated cost per non-empty provider roster, to EVERY planned cell", () => {
    const noJudge = runnerPriceGrid()([{ key: "cell-1", armId: "D" }], arms);
    const withJudge = runnerPriceGrid(undefined, { judgeModels: { anthropic: ["claude-sonnet-4-6"], openai: ["gpt-5.6-luna"] } })(
      [{ key: "cell-1", armId: "D" }],
      arms,
    );
    assert.ok(withJudge.usd > noJudge.usd, "the pre-flight projection must price the planned judging on top of generation");
    assert.ok(withJudge.breakdown[0].byProvider.openai > 0, "an OpenAI judge roster contributes to the openai bucket even on an all-Anthropic generating arm (D)");
  ...
  test("an empty provider roster in judgeModels contributes nothing for that provider", () => {
    const priceGrid = runnerPriceGrid(undefined, { judgeModels: { anthropic: ["claude-sonnet-4-6"], openai: [] } });
    const { breakdown } = priceGrid([{ key: "cell-1", armId: "D" }], arms);
    assert.ok(!("openai" in breakdown[0].byProvider), "an empty openai roster must not fabricate an openai bucket");
  });
});
