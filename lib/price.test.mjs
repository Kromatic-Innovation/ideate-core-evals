...
describe("RATE_TABLE — every entry carries a source and a date", () => {
  ...
  test("OpenAI rates are marked verified, with the check recorded in OPENAI_PRICE_VERIFICATION", () => {
    assert.equal(RATE_TABLE["openai-mid-tier"].verified, true);
    assert.equal(RATE_TABLE["openai-large-tier"].verified, true);
    ...
    // The two OpenAI rows this table actually uses must appear, verbatim,
    // in the raw fetch record -- catches a table/record drift bug where
    // someone hand-edits RATE_TABLE without updating the verification log.
    const found = OPENAI_PRICE_VERIFICATION.modelsFoundUsdPerMTok;
    const mid = RATE_TABLE["openai-mid-tier"];
    const large = RATE_TABLE["openai-large-tier"];
    assert.deepEqual(found[mid.resolvedOpenAiModelId], { in: mid.in, out: mid.out });
    assert.deepEqual(found[large.resolvedOpenAiModelId], { in: large.in, out: large.out });
  });
});
...
describe("re-pricing with a changed rate table", () => {
  const rows = [
    ...
    costRow({
      ...
      tokens_by_model: {
        ...
        "openai-mid-tier": { input_tokens: 300_000, output_tokens: 100_000 },
      },
    }),
  ];
  ...
  test("same rows, table A vs table B, produce different totals — no re-run", () => {
    ...
    const tableB = {
      ...
      "openai-mid-tier": { in: 1.0, out: 1.0, source: "test-table-b", date: "2099-01-01" },
    };
  ...
  test("prices every model in tokens_by_model and sums them", () => {
    const row = costRow({
      ...
      tokens_by_model: {
        ...
        "openai-mid-tier": { input_tokens: 1_000_000, output_tokens: 0 },
        "openai-large-tier": { input_tokens: 0, output_tokens: 1_000_000 },
      },
    });
    const priced = priceRow(row);
    ...
    assert.ok(Math.abs(byModelId["openai-mid-tier"].finalUsd - 2.0) < 1e-9);
    // openai large: 1M out @ $30/MTok = $30
    assert.ok(Math.abs(byModelId["openai-large-tier"].finalUsd - 30.0) < 1e-9);
