 ...
 *  per-family branch.
 *
 *  `pruned-cell` joined in #115, on the SAME grammar (`|attempt=N`, not the
 *  old family-private `|pruned=N` suffix) rather than as a fourth scheme.
 *  Doing so retired `salvageEvictedCellSpend`'s scan-from-zero slot
 *  allocator: that allocator was safe only while slots were never freed, and
 *  compaction frees them the moment this family can fold. It now allocates
 *  through this module's own `nextAttemptNumber` (max+1 across raw AND
 *  compacted shapes), exactly like every other family here. See
 *  `evals/harness/runner.mjs`'s `salvageEvictedCellSpend` for the
 *  idempotency-identity change that had to move WITH the allocator (a
 *  compacted pruned-cell record carries `prunedFromStoredAts`, the set of
 *  physical-record identities it folded, precisely because the old
 *  per-record `prunedFromStoredAt` field does not survive a fold).
 *
...
export const ATTEMPT_FAMILIES = ["generation-attempt", "metrics-attempt", "judge-call", "pruned-cell"];
