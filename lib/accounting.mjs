 ...
 *
 * ── `pricing_regime` (issue #119) ────────────────────────────────────────
 * Batch-vs-single is a SECOND, INDEPENDENT axis from `billing_mode`:
 * `billing_mode` is the metering regime ("api" real money vs "subscription"
 * notional), while `pricing_regime` is which read-time rate a call's tokens
 * must be priced at ("batch" API pricing, roughly half of "single"/realtime
 * pricing — see lib/price.mjs's BATCH_DISCOUNT_BY_MODEL). Before this field
 * existed, batch-vs-single was a lever the CALLER passed into
 * `lib/price.mjs`'s `priceRow`/`priceRows` at READ time, applied identically
 * to every row in the store — correct only while a whole store held one
 * regime, which stopped being true once #103's resume path (and the
 * documented `--no-batch` fallback) made mixed-regime stores routine. See
 * lib/store.mjs's `pricingLever` header for the full incident.
 *
 * OPTIONAL, not required: a row omitting it is a LEGACY row (written before
 * this field existed, or by a caller for which the axis genuinely does not
 * apply) — `lib/price.mjs`'s `priceRow`/`priceRows` handle that case
 * explicitly via a named, recorded fallback (`LEGACY_PRICING_REGIME_FALLBACK`)
 * rather than guessing silently. It is validated when GIVEN, exactly like
 * `model`/`tokens_by_model`'s own optional-but-validated shape below.
 *
 * @param {object} o
 ...
 *   @param {"batch"|"single"} [o.pricing_regime]  which read-time rate this
 *                                      call's tokens must be priced at — a FACT
 *                                      about the call, recorded here rather than
 *                                      supplied by whoever happens to be reading
 *                                      the ledger later (issue #119)
 ...
 */
export function costRow({ cellKey, timestamp, billing_mode, pricing_regime, model, tokens_by_model, ...tokens }) {
  ...
  if (pricing_regime !== undefined && pricing_regime !== "batch" && pricing_regime !== "single") {
    throw new Error(
      `costRow: pricing_regime must be "batch" or "single" when given, got ${JSON.stringify(pricing_regime)} -- ` +
        "omit it entirely for a row that genuinely predates/does not carry this fact, never pass a placeholder value.",
    );
  }
  ...
  // Present only when the caller supplied it (undefined stays OFF the row
  // entirely, not written as `null`) -- absence IS the "legacy row" signal
  // lib/price.mjs's fallback path keys on.
  if (pricing_regime !== undefined) row.pricing_regime = pricing_regime;
