...
// arms.config.json's OpenAI slots use placeholder model ids ("openai-mid-tier",
// "openai-large-tier") pending the OpenAI provider adapter (see that file's
// _modelIdSource comment and openaiModelId markers). This module cannot price
// a model ID that doesn't exist in the vendor's price list, so it maps those
// placeholders to real, currently-listed OpenAI model IDs chosen to sit in
// the same relative tier the pre-registration's §8.1 range implied
// ("~2.00-5.00 in / ~12.00-30.00 out") -- see OPENAI_PRICE_VERIFICATION for
// why gpt-5.6-terra / gpt-5.6-sol were the pick, not an older GPT-5 family
// member. When the real provider adapter lands and arms.config.json is
// updated with real openaiModelId values, this table's OpenAI keys should be
// renamed to match -- the placeholder keys stay here only so
// runnerPriceGrid() can price today's arms.config.json without a second
// translation layer.
const RATE_TABLE_KEY_NOTE =
  "Keys match the exact model id strings arms.config.json uses today (its arm slots' `model` field, e.g. 'claude-opus-5', 'openai-mid-tier') -- not the vendor's own wire id where the two differ. This lets callers look up a row's `model` (or a tokens_by_model key) directly with no translation step.";
...
export const RATE_TABLE = {
  ...
  //
  // arms.config.json's "openai-mid-tier" / "openai-large-tier" placeholders
  // are mapped to gpt-5.6-terra / gpt-5.6-sol -- OpenAI's current
  // (2026-07-31) mid- and flagship-tier general-purpose models, matching the
  ...
  "openai-mid-tier": {
    ...
    date: "2026-07-31",
    resolvedOpenAiModelId: "gpt-5.6-terra",
    verified: true,
  },
  "openai-large-tier": {
    in: 5.0,
    ...
    source: "https://developers.openai.com/api/docs/pricing (verified against official page; canonical URL -- openai.com/api/pricing redirects here)",
    date: "2026-07-31",
    resolvedOpenAiModelId: "gpt-5.6-sol",
    verified: true,
  },
};
...
export const OPENAI_PRICE_VERIFICATION = {
  ...
  // Which of the models above this table actually uses, and why:
  selection: {
    "openai-mid-tier": {
      chose: "gpt-5.6-terra",
      why: "current-generation mid tier, closest live analogue to the pre-registration's '~2.00-5.00 in' OpenAI-mid-tier placeholder range",
    },
    "openai-large-tier": {
      chose: "gpt-5.6-sol",
      why: "current-generation flagship tier, matches the pre-registration's '~12.00-30.00 out' upper end",
    },
  },
  note:
