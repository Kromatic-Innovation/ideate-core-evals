...
// ── Judge pre-flight estimate (issue #63) ────────────────────────────────────
// docs/PREREGISTRATION.md §8.2 projects the confirmatory grid's cross-judge
// matrix at "2 judges x 432 pools (batched), ~$0.05/pool -> ~$43" -- i.e. a
// flat ~$0.05 per SCHEDULED judge leg (one pool scored by one judge), already
// reflecting batch pricing (the doc's own total is the batched figure, not a
// pre-discount one). This is deliberately a flat USD estimate rather than a
// token x rate-table computation: the judge's real call shape is one
// Anthropic-Messages-style request PER CANDIDATE (score.mjs's
// buildJudgeRequests), so a token-accurate projection would need the pool's
// candidate count -- information a planned CELL (this module's `plannedCells`
// input) does not carry, only a planned POOL does. Using the doc's own
// pool-level dollar projection keeps the pre-flight consistent with the
// number §7/§8.2 already disclose, without inventing an unsourced token count.
const JUDGE_ESTIMATE_USD_PER_LEG = 0.05;
 ...
 * and one OpenAI judge leg PER POOL. Passing `judgeModels` (the same
 * `{anthropic: [...], openai: [...]}` shape score.mjs/matrix.mjs use) adds
 * JUDGE_ESTIMATE_USD_PER_LEG for each non-empty provider roster to every
 * cell's projection and byProvider split -- so `--max-spend`/
 * `--max-spend-<provider>` can see the study's planned JUDGING, not only its
 * planned GENERATION (the gap docs/PREREGISTRATION.md §7 discloses against
 * this issue). Omitting `judgeModels` (the default) leaves every existing
 * caller's projection byte-identical to before this option existed.
 *
 * @param {object} [rateTable=RATE_TABLE]
 * @param {object} [opts]
 *   @param {{anthropic?: string[], openai?: string[]}} [opts.judgeModels]  when
 *     supplied, a non-empty provider roster adds one judge leg's estimated
 *     cost to EVERY planned cell (each cell is a pool the confirmatory matrix
 *     schedules for judging) under that provider's bucket.
 * @returns {(plannedCells: Array, arms: object, opts?: {batch?: boolean}) => {usd: number, breakdown: Array}}
 */
export function runnerPriceGrid(rateTable = RATE_TABLE, { judgeModels } = {}) {
  return function priceGrid(plannedCells, arms, { batch = true } = {}) {
    ...
    for (const cell of plannedCells) {
      ...
      // Judge term: one leg per non-empty provider roster in judgeModels, PER
      // POOL (= per planned cell) -- see the header comment above. Additive
      // only when judgeModels is supplied; a caller that never passes it gets
      // exactly the pre-#63 projection, unchanged.
      if (judgeModels) {
        for (const provider of Object.keys(judgeModels)) {
          const roster = judgeModels[provider];
          if (!Array.isArray(roster) || roster.length === 0) continue;
          cellUsd += JUDGE_ESTIMATE_USD_PER_LEG;
          byProvider[provider] = (byProvider[provider] || 0) + JUDGE_ESTIMATE_USD_PER_LEG;
        }
      }
