...
// NOTE (issue #101): this pins `planRun`'s own mechanics, and no longer
// describes what `evals/run.mjs` does. SPEC.config here never sets
// `armsConfigHash`, but a real run always does — and that hash is over the
// WHOLE arms.config.json, so adding an arm to the file moves it and marks
// every OTHER arm's cells stale too. That consequence is accepted and
// registered (docs/PREREGISTRATION.md Appendix D item 1); see the
// "adding or removing an arm invalidates every arm's cells" test below.
...
test("#101 — adding or removing an arm invalidates EVERY arm's cells (registered consequence)", () => {
  // The costly half of hashing the whole file, pinned so it can never be hit
  // by surprise. Registered in docs/PREREGISTRATION.md Appendix D item 1.
  //
  // Accepted rather than fixed. `spec.config` is per-spec, so a per-arm hash
  // cannot live in it — the same constraint that retired ideasPerAgent and
  // maxRounds. The only finer-grained alternative, hashing just the arms a
  // given spec runs, is strictly worse: it would make arm A's cells from
  // `--arms A,B` incomparable to arm A's cells from `--arms A,C`, shredding
  // the additive design far more severely than over-invalidation does. (The
  // reassuring counterpart — that `--arms` SCOPING does not move the hash —
  // is pinned in evals/run.test.mjs.)
  const before = readArmsConfig();

  const withNewArm = readArmsConfig();
  withNewArm.arms.I = { label: "new arm", mode: "panel", personaDisabled: false, slots: [{ persona: "proposer_1", model: "claude-haiku-4-5" }] };
  assert.notEqual(armsConfigHash(withNewArm), armsConfigHash(before), "adding an arm moves the hash");

  const withoutH = readArmsConfig();
  withoutH.arms = Object.fromEntries(Object.entries(withoutH.arms).filter(([id]) => id !== "H"));
  assert.notEqual(armsConfigHash(withoutH), armsConfigHash(before), "and so does dropping one");

  // The consequence spelled out: arm B is untouched by either edit, and its
  // cells still go stale.
  assert.deepEqual(withNewArm.arms.B, before.arms.B, "arm B's own definition did not change");
  const spec = { arms: [{ id: "B" }], briefs: [{ id: "b1" }], replicates: 1, config: { armsConfigHash: armsConfigHash(before) } };
  const stored = planCells(spec).map((c) => c.key);
  const { reuse, stale } = planRun({ ...spec, config: { armsConfigHash: armsConfigHash(withNewArm) } }, stored);
  assert.equal(reuse.length, 0, "an unrelated arm's cells are NOT reused after another arm is added");
  assert.equal(stale.length, 1, "they go stale — expect a full re-collection, so introduce arms BEFORE a phase begins");
});

// ── issue #129 amendment C: `effort` and per-arm panel overrides participate
// in configHash. Both live in arms.config.json DATA, so `armsConfigHash`'s
// existing denylist-based (not allowlist-based) canonicalization already
// covers them for free -- these tests assert the hash actually MOVES for
// each new field specifically, not merely that some field exists, per the
// brief's "a test asserts the hash actually moves ... not merely that a field
// exists" requirement.

test("#129 — adding a slot-level `effort` moves armsConfigHash AND configHash", () => {
  const before = readArmsConfig();
  const after = readArmsConfig();
  after.arms.C.slots[0].effort = "low";

  const armsHashBefore = armsConfigHash(before);
  const armsHashAfter = armsConfigHash(after);
  assert.notEqual(armsHashAfter, armsHashBefore, "an added effort field must move armsConfigHash");

  const cfgBefore = configHash({ harnessVersion: "0.1.0", engineSha: "x", armsConfigHash: armsHashBefore });
  const cfgAfter = configHash({ harnessVersion: "0.1.0", engineSha: "x", armsConfigHash: armsHashAfter });
  assert.notEqual(cfgAfter, cfgBefore, "an effort-only difference must move configHash end-to-end");
});

test("#129 — changing effort's VALUE (not merely presence) also moves both hashes", () => {
  const low = readArmsConfig();
  low.arms.C.slots[0].effort = "low";
  const high = readArmsConfig();
  high.arms.C.slots[0].effort = "high";

  assert.notEqual(armsConfigHash(high), armsConfigHash(low), "different effort VALUES must hash differently, not just presence-vs-absence");
});

test("#129 — a per-arm panel override (ideasPerAgent/maxRounds/sharing) moves armsConfigHash", () => {
  const before = readArmsConfig();
  const withOverride = readArmsConfig();
  withOverride.arms.C.panel = { ideasPerAgent: 3, maxRounds: 1, sharing: "blind" };
  assert.notEqual(armsConfigHash(withOverride), armsConfigHash(before), "an arm-level panel override must move armsConfigHash");
});

test("#129 — registering arm AS (the stance-neutral solo control) moves armsConfigHash for the WHOLE file, not just arm AS", () => {
  const before = readArmsConfig();
  assert.ok(before.arms.AS, "arm AS must already be registered on develop for this test to be meaningful");
  const withoutAS = readArmsConfig();
  delete withoutAS.arms.AS;
  assert.notEqual(armsConfigHash(before), armsConfigHash(withoutAS), "arm AS's registration is part of what the current hash covers");
});
