...
| **Fluency / Flexibility**   | LiveIdeaBench axes: count of valid candidates emitted; breadth of distinct categories (clusters) covered — POOL properties, computed here as operational metrics (`evals/metrics/operational.mjs` `poolFluency`/`poolFlexibility`), not scored per idea by the judge (corrected issue #45 item 2 — an earlier draft had the per-idea judge score these, which is undefined for a scorer that sees one candidate at a time and never the pool) |

### 4.2 Secondary (idea-level, split axes — never collapsed)

| Metric                         | Notes                                                                                                                                                                                                                                                                                     |
| ------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Novelty** (judged, 1–10)     | Split-axis per Rietzschel et al. 2010 — _never_ averaged with feasibility into a single "best". Scale corrected to match `evals/judge/prompt.mjs` (issue #45 item 1: code and registration must agree; 1–10 kept for the judge's extra resolution, rather than narrowing the code to 1–5) |
| **Feasibility** (judged, 1–10) | Same                                                                                                                                                                                                                                                                                      |
