...
**A reportable finding in its own right.** This is registered as a finding, not merely absorbed as a limitation: **the released anonymized Si et al. review data cannot support LLM-judge validation at useful precision**, because it ships no reviewer identifier and most ideas (105 of 147) carry exactly two reviews. It is checkable, it concerns a widely-cited public dataset, and — unlike the per-dollar model ranking this study produces — it does not expire when the model lineup changes.

This entry does not independently re-run #47's bootstrap; the numbers above are read verbatim from the committed `docs/si-et-al-human-human-floor.json`.
...
| R3   | no confirmatory inference; study reported as **descriptive** | R0–R2 all fail, or sidecar unavailable                                     |
...
**Verified.** `evals/analysis/fit.mjs` implements exactly this ladder (`runLadder`, `fitViaSidecar` for R0/R1, `fitR2` for the CR2 fallback), including the R3-vs-sidecar-unavailable distinction the table's last row states, and the CI job **"Analysis sidecar (ANALYSIS_SIDECAR=1)"** (`.github/workflows/ci.yml`) runs the real statsmodels REML fit against the closed-form ANOVA oracle (`evals/analysis/fit.integration.test.mjs`, `anova-oracle.mjs`) — confirmed present and green on `develop` at the time of this writing. That job is a **separate CI job with its own Python venv**, distinct from the default `node --test` job (which never sets `ANALYSIS_SIDECAR` and so never exercises the real sidecar comparison). With that job green, the statistical core is verified on every push to `develop`; without it, `node --test` alone would only verify "the code ran," not "the statistics were checked."
...
**The 3 original scientific keywords are preserved as a prefix under the same seed** — only 3 additional scientific briefs were appended, none of the original 3 were re-rolled — so the previously-registered scientific briefs are provably unchanged; a reader can diff `evals/corpus/briefs.mjs`'s scientific stratum and see the original 3 entries untouched at the head of the list.
